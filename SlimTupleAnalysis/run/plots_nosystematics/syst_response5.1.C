...
#include "calculateRatioError.h"
  ...
  for (int isyst=0;isyst<nsyst;isyst++){
    ...
    fsyst[isyst]=TFile::Open(Form("~azaman/Analysis5/SlimTupleAnalysis/run/plots_nosystematics/RATIO.caf4.2.lv8_smw_wp20_NEW/ratio_%s.caf4.2_lv8_smw_wp20_NEW.root",systname[isyst].Data())); //&&&& only FatJESUp and JMSUp
  ...
  for (int iHist=0;iHist<nHist;iHist++){
    for (int iRegion=0;iRegion<nRegion;iRegion++){	  
	  ...
	  for (int isyst=0;isyst<nsyst;isyst++){
	    ...
	    // /*
	    h_Nom[isyst] =(TH1F*)fsyst[isyst]->Get(Form("H_%s_%s_WBF_%s_%s_Nominal", sampleName[iSample].Data(), flavorName[iFlavor].Data(), regionName[iRegion].Data(), histName[iHist].Data()) );
	    h_Syst[isyst] =(TH1F*)fsyst[isyst]->Get(Form("H_%s_%s_WBF_%s_%s_syst", sampleName[iSample].Data(), flavorName[iFlavor].Data(), regionName[iRegion].Data(), histName[iHist].Data()) );
	    h_both[isyst] =(TH1F*)fsyst[isyst]->Get(Form("H_%s_%s_WBF_%s_%s_both", sampleName[iSample].Data(), flavorName[iFlavor].Data(), regionName[iRegion].Data(), histName[iHist].Data()) );
	    h_NomOnly[isyst] =(TH1F*)fsyst[isyst]->Get(Form("H_%s_%s_WBF_%s_%s_NominalOnly", sampleName[iSample].Data(), flavorName[iFlavor].Data(), regionName[iRegion].Data(), histName[iHist].Data()) );
	    h_SystOnly[isyst] =(TH1F*)fsyst[isyst]->Get(Form("H_%s_%s_WBF_%s_%s_systOnly", sampleName[iSample].Data(), flavorName[iFlavor].Data(), regionName[iRegion].Data(), histName[iHist].Data()) );
	    
	    
	    ratioHist2[isyst] = (TH1F*) h_Syst[isyst]->Clone();
	  ...
	  ratioHist2[0]->GetYaxis()->SetRangeUser(-3., 5);
	  ...
	  l.DrawLatex(0.15 , 0.80,"#int Ldt = 20.3 fb^{-1}");
	  ...
	  //c1->Print(Form("PDF_syst_band/response_%s_%s_%s_%s_4.pdf", sampleName[iSample].Data(), flavorName[iFlavor].Data(), regionName[iRegion].Data(), histName[iHist].Data()));
