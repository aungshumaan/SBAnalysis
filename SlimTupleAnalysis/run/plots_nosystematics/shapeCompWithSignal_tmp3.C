...
void shapeCompWithSignal_tmp3(){
  ////shape comparison with ttbar, W+jets, signal SM and signal aQGC

  const int nFlav=1;TString flavorName[nFlav]={"l"};
  ...
  TFile *f=TFile::Open("~azaman/Analysis5/SlimTupleAnalysis/run/hwwlvqq.Nominal.caf4.2.lv4_smw_noSeparateWjetsEWK.root"); //&&&& mjjTagged>500
  ...
  for (int itype=0;itype<nType;++itype){
	...
	//h_qcd=(TH1F*)f2->Get(Form("H_QCD_ep_WBF_%s_%s", cutName[icut].Data(),typeName[itype].Data()));
	//h_qcd->Add((TH1F*)f2->Get(Form("H_QCD_em_WBF_%s_%s", cutName[icut].Data(),typeName[itype].Data())),s);
	...
	h_WV_0_03=(TH1F*)f->Get(Form("H_VBS_WV_0_03_ep_WBF_%s_%s", cutName[icut].Data(),typeName[itype].Data()));
	h_WV_0_03->Add((TH1F*)f->Get(Form("H_VBS_WV_0_03_em_WBF_%s_%s", cutName[icut].Data(),typeName[itype].Data())),s);
	...
	*/
	...
	//c2->Print(Form("plots_shape_withSig.pdf" ),Form("Title:%s_%s_%s",cutName[icut].Data(),typeName[itype].Data()));
