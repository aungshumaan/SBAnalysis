...
void syst_band_fat_smoothed(){ 
  ...
  for (int icut=0;icut<nCut;++icut){
	...
	if (typeName[itype].Contains("bosonCentrality") && cutName[icut].Contains("fatOnly") ) {
	  hs->GetYaxis()->SetNdivisions(210); //&&&& fat bosoncen
	}
	h_data->SetMarkerStyle(20);
	h_data->SetMarkerSize(1);
	...
	*/
	...
	//TArrow *ar1 = new TArrow(xcut,ymax*(0.7),xcut*(1.5),ymax*(0.7),0.02,"|>");
	TArrow *ar1 = new TArrow(xcut,ymax*(0.95),xcut*(1.5),ymax*(0.95),0.02,"|>");
	//TArrow *ar1 = new TArrow(xcut,ymax*(0.95),xcut*(0.8),ymax*(0.95),0.02,"|>");  //cosThetaStar, Awv
	ar1->SetLineWidth(2);
